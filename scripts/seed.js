...
import { applySchema, getDb, closeDb, transaction } from '../src/infrastructure/database/connection.js';
import config from '../src/config/index.js';
import { ALL_PERMISSIONS, ROLE_DEFINITIONS } from '../src/shared/permissions.js';
...
transaction(() => {
  ...
  for (const p of ALL_PERMISSIONS) {
    insertPermission.run(p.code, p.module, p.action, `${p.action} in ${p.module}`);
  }
  ...
  for (const role of ROLE_DEFINITIONS) {
    insertRole.run(role.code, role.name_en, role.name_ar, role.description);
    const roleId = db.prepare('SELECT id FROM roles WHERE code = ?').get(role.code).id;
    db.prepare('DELETE FROM role_permissions WHERE role_id = ?').run(roleId);
    for (const code of role.permissions) linkPermission.run(roleId, code);
  }
  ...
  insertSequence.run('sale', 'INV', 5, year);
  insertSequence.run('purchase_order', 'PO', 5, year);
  insertSequence.run('sales_return', 'RET', 5, year);
  insertSequence.run('stock_adjustment', 'ADJ', 5, year);
  ...
  for (const [key, value, type, group] of settings) insertSetting.run(key, value, type, group);

  // Administrator
  const adminRoleId = db.prepare("SELECT id FROM roles WHERE code = 'admin'").get().id;
  const locationId = db.prepare("SELECT id FROM warehouses WHERE code = 'MAIN'").get().id;
  ...
  `);
  insertAttribute.run('SIZE', 'Size', 'المقاس', 'select', 1);
  insertAttribute.run('COLOR', 'Colour', 'اللون', 'color', 2);
  insertAttribute.run('MATERIAL', 'Material', 'الخامة', 'select', 3);

  const attributeId = (code) => db.prepare('SELECT id FROM attributes WHERE code = ?').get(code).id;
  ...
  ].forEach(([code, en, ar], i) => insertValue.run(attributeId('SIZE'), code, en, ar, null, i));
  ...
  ].forEach(([code, en, ar, hex], i) => insertValue.run(attributeId('COLOR'), code, en, ar, hex, i));
  ...
  ].forEach(([code, en, ar], i) => insertValue.run(attributeId('MATERIAL'), code, en, ar, null, i));
});
...
function seedExample() {
  transaction(() => {
    ...
    const attributeId = (code) => db.prepare('SELECT id FROM attributes WHERE code = ?').get(code).id;
    const valueId = (attrCode, valCode) => db.prepare(`
    ...
    db.prepare(`
      ...
      VALUES ('cashier', 'Youssef Hany', 'cashier@mm-accessories.local', '+20 102 222 2222',
              ?, ?, ?, 'en', 1, ?)
    `).run(bcrypt.hashSync('cashier123', config.auth.bcryptRounds), roleId('cashier'), locationId, adminId);
    const cashierId = db.prepare("SELECT id FROM users WHERE username='cashier'").get().id;

    // --- one supplier
    const supplierId = db.prepare(`
    ...
    db.prepare('INSERT INTO product_attributes (product_id, attribute_id, display_order) VALUES (?, ?, 0)')
      .run(productId, attributeId('SIZE'));
    db.prepare('INSERT INTO product_attributes (product_id, attribute_id, display_order) VALUES (?, ?, 1)')
      .run(productId, attributeId('COLOR'));
    ...
    for (const size of sizes) {
      for (const colour of colours) {
        ...
        const variantId = insertVariant.run(
          productId, sku, sku, `${size.label} / ${colour.label}`,
          620, price, Math.round(price * 0.8), 3, 12,
        ).lastInsertRowid;
        insertOption.run(variantId, attributeId('SIZE'), valueId('SIZE', size.code));
        insertOption.run(variantId, attributeId('COLOR'), valueId('COLOR', colour.code));
    ...
    for (const variant of variants) {
      insertPoLine.run(poId, variant.id, orderQty, orderQty, unitCost, lineNet * 1.14);
      insertLevel.run(variant.id, locationId, orderQty, unitCost);
      insertMovement.run(variant.id, locationId, 'purchase_receipt', orderQty, unitCost,
    ...
    db.prepare('INSERT INTO sale_payments (sale_id, amount, method, created_by, paid_at) VALUES (?, ?, ?, ?, ?)')
      .run(saleId, total, 'cash', cashierId, daysAgo(2));

    db.prepare('UPDATE stock_levels SET quantity = quantity - ? WHERE variant_id = ? AND warehouse_id = ?')
      .run(soldQty, soldVariant.id, locationId);
    insertMovement.run(soldVariant.id, locationId, 'sale', -soldQty, unitCost,
      orderQty - soldQty, 'sale', saleId, invoiceNo, cashierId, daysAgo(2));

    db.prepare('UPDATE customers SET loyalty_points = ? WHERE id = ?')
      .run(Math.floor(total * 0.05), customerId);

    db.prepare('UPDATE sequences SET next_value = 2 WHERE name IN (?, ?)').run('sale', 'purchase_order');
    db.prepare('UPDATE sequences SET next_value = 2 WHERE name = ?').run('purchase_order');
