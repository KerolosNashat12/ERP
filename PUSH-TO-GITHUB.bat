...
git add -A
git commit -m "Backups: raise the 64 MB stored ceiling to 200 MB, fix three tests that only worked by inheriting an old seeded database, add Export CSV to the Products screen, raise the product photo size limit from 400 KB to 5 MB (plus the request body limit that would have quietly capped it at 3.5 MB), and speed up the storefront home page by inlining small brand logos as data URIs instead of one network request per brand. Then raised the backup ceiling again, to 400 MB, when 200 MB was crossed within 48 hours of the photo-size change going out - the shop's last good backup was 128 MB on Sep 7, the next scheduled runs read 251-252 MB and failed the same way; if a third crossing happens the photo-size ceiling itself is worth reconsidering rather than raising this number a third time"
if errorlevel 1 (
  echo.
