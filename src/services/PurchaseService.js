...
export class PurchaseService {
  ...
  get(id) {
    const order = this.orders.findAggregate(id);
  ...
  save(payload, context = {}, orderId = null) {
    return transaction(() => {
      this.suppliers.requireById(payload.supplier_id, 'supplier');
      ...
      let existing = null;
      if (orderId) {
        existing = this.orders.requireById(orderId, 'purchase order');
      ...
      const data = {
        ...
        warehouse_id: payload.warehouse_id || this.inventory.locationId(),
      ...
      let order;
      if (orderId) {
        order = this.orders.update(orderId, data);
      } else {
      ...
      const previous = orderId ? this.orders.lines(orderId) : [];
      ...
      this.orders.replaceLines(order.id, merged);
   ...
   */
  receive(id, { receipts = [], notes } = {}, context = {}) {
    return transaction(() => {
      const order = this.orders.findAggregate(id);
      if (!order) throw new NotFoundError('Purchase order', id);
      ...
      for (const receipt of toReceive) {
        ...
        this.orders.updateLineReceived(line.id, round3(line.quantity_received + quantity));
        // Keep the variant's standard cost aligned with the latest purchase price.
        this.variants.update(line.variant_id, { cost_price: round2(line.unit_cost) });
      }

      const refreshed = this.orders.lines(id);
  ...
  cancel(id, reason, context = {}) {
    return transaction(() => {
      ...
      const updated = this.orders.update(id, { status: 'cancelled', notes: reason || order.notes });
      this.audit.record({
  ...
  registerPayment(id, { amount, method = 'cash', reference }, context = {}) {
    return transaction(() => {
      ...
      const updated = this.orders.update(id, { paid_amount: paid });
      this.audit.record({
  ...
  suggestReorder(warehouseId) {
    const lowStock = repositories.inventory.lowStock(warehouseId, 500);
    ...
    for (const row of lowStock) {
      ...
      const variant = this.variants.findById(row.variant_id);
