...
echo   Committing...
git add -A >> "%LOG%" 2>&1
git -c user.email="kerolosnashatestfanous@gmail.com" -c user.name="KerolosNashat12" commit -m "The 404 is gone and the backup got one step further, to a limit that is Turso's and not SQLite's: the consistency signature was one UNION ALL over all forty-four tables - one round trip whatever the shop's size, which was the point - and SQLite allows five hundred compound terms and ran it without blinking. The hosted database answered SQLITE_UNKNOWN: SQLite error: too many terms in compound SELECT. Same lesson as ANALYZE, learned a second time: the server sets ceilings the local file does not, and no test against a file database can find them. The signature is taken eight tables at a time now, and falls back to one table per statement if even that is too many somewhere. The split is invisible by construction - the rows are sorted before they are hashed, so the same shop signs the same whether that took one statement or forty, and there is a test that holds it to that." >> "%LOG%" 2>&1
