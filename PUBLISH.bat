...
echo   Committing...
git add -A >> "%LOG%" 2>&1
git -c user.email="kerolosnashatestfanous@gmail.com" -c user.name="KerolosNashat12" commit -m "Five things the owner found by looking at his own shop. The buy box had no shared edge - add to cart stretched to whatever was left of its row while save to favourites underneath hugged its own words - so it is one capped column now and the three controls line up. The option picker was nine chips each sized to its own label, a staircase, every one of them printing the same 1,000 under a heading that already said 1,000 in the largest type on the page: equal columns, the chosen option named in the field label, the option's own photograph as a swatch where the shop uploaded one, and a price on a chip only where it genuinely differs. The main photograph is fitted instead of cropped, because these are supplier shots of tall bottles on white and a square crop takes the cap off the top and the name off the bottom; fitting can only add white to a picture already on white. A product with no photograph now says so instead of showing a screen of blank white that reads as a failure. The brands section was sixty text pills wrapped over six rows - it is a rail that scrolls, each brand carrying its logo where brands.logo_url has one recorded and its first letter where it does not, plus how many pieces are behind it. Deliberately not a marquee: a moving row of links has to be caught before it can be clicked. And the discount on a purchase order is a RATE - a supplier says five percent, not forty pounds - so the field asks for the rate, the amount is worked out from the subtotal and follows the lines when they change, and discount_amount stays the money that every total and report already reads. Orders saved before this keep their amount untouched." >> "%LOG%" 2>&1
