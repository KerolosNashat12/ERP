...
echo   Committing...
git add -A >> "%LOG%" 2>&1
git -c user.email="kerolosnashatestfanous@gmail.com" -c user.name="KerolosNashat12" commit -m "A shop that is open for business could not be backed up, because the check refused every write rather than the ones that matter: three attempts in a row died on SnapshotRaceError while the owner was editing his own catalogue in the next tab. Not every concurrent write can tear an archive. Each table is now read only up to the highest rowid it had when the read began, so a sale rung up in the middle is in NO part of the copy - not the invoice, not its lines, not the stock movements - instead of being in some tables and missing from others. The tear is prevented by construction rather than detected afterwards, which leaves the two things a ceiling cannot cover. An edit in place is allowed and recorded in the manifest, because every reference still points at something that exists and holding a row as it was thirty seconds ago is what a backup IS. A delete still fails it, because a sale removed after its invoice was read but before its lines were leaves lines belonging to nothing - and now the failure names the table and the counts, so the next one is evidence instead of a verdict. Also: the retention line read 14 nightly, 50 by hand, 30 before a restore. It is 5 and 3. The separator was a middot, and in a right-to-left line it settles against the digit before it and is read as a zero." >> "%LOG%" 2>&1
