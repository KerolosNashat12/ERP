...
echo   Committing...
git add -A >> "%LOG%" 2>&1
git -c user.email="kerolosnashatestfanous@gmail.com" -c user.name="KerolosNashat12" commit -m "الهدر has a screen now, and the fleet console can finally see what a shop lost. The money side landed last round - broken, lost, stolen and expired stock valued at cost, on the dashboard and in the profit report - but recording one still meant opening a stock count, adding a line, working out what the shelf holds and typing what is LEFT rather than what was lost: five steps and a subtraction, for something that happens with a dustpan in the other hand. It is one line now - the piece, how many, and why - and the document underneath is the same stock adjustment with the same movement and the same audit trail, because a second mechanism for taking stock off a shelf is a second version of the truth. It refuses to write off more than is on the shelf: a shop that has lost more than it had has counted something wrong, and a negative shelf hides that instead of surfacing it. The owner console gained the other half of the picture on a shop's report - kept after refunds, refunded, and wastage, beside the takings rather than folded into them, because from up there a shop that refunded most of what it sold looked identical to one that kept all of it. And the storefront: the products and the categories are back to a grid. Every shelf was briefly a rail for consistency with the brands; the owner looked at it and said put the products back, and he was right - a rail is for a long row nobody reads in order, which sixty brand logos are and eight new arrivals are not. They come back better than they left: five columns instead of four locked ones, so a card is the size of the thing it is showing rather than 280px of picture before a word of it is read." >> "%LOG%" 2>&1
