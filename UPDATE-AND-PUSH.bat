...
title M^&M Accessories ERP - update and publish
cd /d "%~dp0"
...
echo.
echo   M^&M Accessories - update and publish
echo   ==========================================
...
if errorlevel 1 (
  echo   [X] Could not unpack - see update-log.txt
  echo.
  pause
  exit /b 1
)
...
echo   Committing...
git add -A >> "%LOG%" 2>&1
git -c user.email="kerolosnashatestfanous@gmail.com" -c user.name="KerolosNashat12" commit -m "Online shop: public storefront at /shop, product photos, cash-on-delivery orders confirmed by staff, migration system" >> "%LOG%" 2>&1
...
echo       or try a phone hotspot. Details: update-log.txt
echo.
pause
exit /b 1
